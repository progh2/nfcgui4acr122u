...
- **웹사이트(URL)** — NDEF URI 레코드로 주소를 기록. 탭하면 브라우저에서 해당 사이트가 열림
  - 스킴(http/https)이 없으면 `https://`를 자동으로 붙이고, 접두어를 코드로 압축해 공간 절약
...
- **일괄 굽기** — 같은 URL/텍스트/WiFi를 여러 태그에 연속으로 자동 기록 (학급 세트 제작용)
  - 시작 후 태그를 하나씩 올렸다 빼면 자동 기록. 같은 태그(UID)는 한 번만.
- **태그 관리** — 태그 초기화(사용자 데이터 지우기), 영구 잠금(읽기전용, 되돌릴 수 없음)
